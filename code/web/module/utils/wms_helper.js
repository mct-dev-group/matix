...
define(function(){
   ...
   * @param {*} callback 
   */
    this.getRect = function (bbox, width, height, callback) {
      const {url, layers, format, srs} = this.option;
      const urlStr = `${url}?SERVICE=WMS&VERSION=1.1.1&REQUEST=GetMap&FORMAT=${format}&TRANSPARENT=true&LAYERS=${layers.join(',')}&SRS=${srs}&STYLES=&WIDTH=${width}&HEIGHT=${height}&BBOX=${bbox}`;
    ...
    this.getRectImage = function(bbox, width, height, callback) {
      const {url, layers, format, srs} = this.option;
