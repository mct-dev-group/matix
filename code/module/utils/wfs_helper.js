define(function() {
  var WFSHelper = function (option) {
    this.option = option;
    this.getFeature = function(typenames,CQL_FILTER, callback) {
      let funcArr = []
      for (let typename of typenames.split(',')) {
        funcArr.push(wfsQuery(typename,CQL_FILTER));
      }
    ...
    var self = this;
    function wfsQuery(typename,CQL_FILTER) {
      ...
      return new Promise((resolve, reject) => {
        ...
        const urlStr = `${url}?service=WFS&request=GetFeature&version=${version}&typename=${typename}&CQL_FILTER=${CQL_FILTER}${outputformat}`;
